...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className='flex w-full'>
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        'flex w-full min-w-0 flex-1 items-center justify-between py-4 font-medium transition-all hover:underline [&[data-state=open]>svg]:rotate-180',
        className,
      )}
      {...props}
    >
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Content
    ...
    {...props}
  >
    <div className={cn('w-full min-w-0 pb-4 pt-0', className)}>{children}</div>
  </AccordionPrimitive.Content>
))
